...
********************************
------------------------------*/
#define FWVersion		"ver.09/15.1.00.33"
	// без этой версии последняя версия 33		// 34 спец Версия для Казани с вкл CheckConfig !!!
												// 33 исправлен расчет мин открытия экрана
												// 32 контроль мак ветра и внешней темп, не работаем фрамугами пока нет температуры для вент
												// 31 починил экран работу по заданию
												// 30 починил отправку ручного управления
												// 29 изменен алгоритм для экрана в нижнем диап работы
												// 28 исправления для экрана
												// 25 проверка пакетов CRC
												// 21_7  без CRC, но с проверкой checkConfig
												// 21_6  CRC больших пакетов не считаем, если нет т контура 0 не берем в расчет
												// 21_5  CRC пакета ethrnet в том числе и заголовка + CRC считается только для пакетов IN
												// 21_4  CRC пакета ethrnet в том числе и заголовка + CRC считается только для пакетов IN
												// 21_3  CRC пакета ethrnet в том числе и заголовка
												// 21_2  CRC пакета ethrnet
