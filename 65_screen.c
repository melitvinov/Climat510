...
//Оптимицация на typScr
    ...
    case 0:
        ...
        if (!bZad) {
            if (IntZ<GD.TuneClimate.sc_TOutClose) pScr->Mode=1;
            if ((IntZ>GD.TuneClimate.sc_TOutClose+200)||(!GD.TuneClimate.sc_TOutClose)) pScr->Mode=0;
...
void LaunchVent(void)
{
    ...
    if (pGD_TControl_Tepl->PauseVent) {
        pGD_TControl_Tepl->PauseVent--;return;
    }
...
{
    if (!(pGD_MechConfig->RNum[cHSmHeat])) {
        pGD_TControl_Tepl->Calorifer=0;return;
    }
