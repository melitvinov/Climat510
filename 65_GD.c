...
{
    uchar       Zone[1];        // зона
    uint16_t    TimeStart;      // время начала
    uint8_t     TypeStart;      // тип старта    ??????????????????????
    uint8_t     Rez1;           // 
    uint16_t    TAir;           // темп отопления
    uint16_t    TVentAir;       // темп вентиляции
    uint16_t    CO2;            // уровень CO
                                //
    uchar       TOptimal1;      // оптимум контура 1
    uchar       TOptimal2;      // оптимум контура 2
                                // 
    uchar       TPipe3;         // фиксированная темп 3 контура
    uchar       TPipe4;         // фиксированная темп 4 контура
                                //
    uchar       MinTPipe5;      // минимальная темп 5 контура
                                //
    uchar       MinTPipe1;      // минимальная темп 1 контура
    uchar       MinTPipe2;      // минимальная темп 2 контура
                                //
    uchar       MinOpenWin;     // мин открытие фрамуг
    uchar       Win;            // 3-и режима работы фрамуг
                                //
    uint16_t    SIO;            // кол-во сек распыления (для повышения влажности)
                                //
    uchar       Screen[3];      // режим работы экрана
    uchar       Vent;           // режим работы вентиляции вкл/выкл
                                //
    uchar       Light;          // % от мощьности подсветки
    uchar       ModeLight;      // режим работы светильников
                                //
    uchar       MinTPipe3;      // третья минимальная температура 
    uchar       RHAir_c;        // влажность
    uint16_t    Rez[10];        // резерв
}
eTimer;
...
{
//0
    uchar       News;                           // новости, события 
    /*Блок пиема из ПК*/                        // 
    eSensing    MeteoSensing[cConfSMetSens];    // изменеия наружних датчиков
    int16_t     MaxReqWater;                    // макс температура воды (котельная)
    int16_t     HeatPower;                      // ???
    int16_t     FullHeat;                       // ???
//40                                            //
    int16_t     NextRHAirSens;                  // не используется
    int16_t     Vosx;                           // восход, кол-во минут от начала суток
    int16_t     Zax;                            // закат, кол-во минут от начала суток
/***************************************/       //
    int16_t     SumSun;                         // кол-во Дж сумарно за сутки
    uchar       Regs[5];                        // ???
    uchar       Util;                           // ???
//50                                            //
    uchar       isLight;                        // вкл/выкл свет
    uchar       blockCO2;                       // управляется со стороны компьютера
    uchar       Rez[4];                         // резерв
    int16_t     MidlSR;                         // усредненый 
    int16_t     MidlWind;                       // усредненый
    int16_t     PozFluger;                      // 0 - ветер с севера, 1 - с юга
                                                //
    uint16_t    Time;                           // 
    uint16_t    Data;                           //
    uchar       Year;                           //
...
typedef struct eeTeplControl
{
    ...
    int16_t     c_MinTPipe[2];              // миним темп 3 или 4 контура
    int16_t     c_DoPres;                   // регулирование перепада давления
    int16_t     c_OptimalTPipe[2];          // оптимальная температура 3 или 4 контура
    int16_t     f_IFactor;                  // интегральный фактов фрамуг (винтеляция)
    int16_t     c_PFactor;                  // Контур 1 - Динамика(Тзад-Тизм)влияет до  Пропорциональный фактор
    int16_t     c_IFactor;                  // Интегральная поправка контуров           Интегральный фактор
    ...
    int16_t     sc_TMaxOpen;                // макс открытие экрана. изоляция от солнца
    int16_t     sc_ZMaxOpen;                // макс открытие экрана. затеняющий экран 
    uint16_t    co_model;                   // СО2 - исполнитель(0-регулятор,1-клапан)
    uint16_t    sio_SVal;                   // кол-во клапанов испарительного охлаждения
    uint16_t    sLight;                     // кол-во градаций досветки
    int16_t     Rez[20];
//+42 байта
} eTeplControl;
...
{
    uchar           rModification;      // используется при идетификации типа контроллера
    uchar           rSInTeplSens;       //
    uchar           rSMechanic;         //
    uchar           rSTepl;             // кол-во зон
    uchar           rVersion;           // используется при идетификации типа контроллера
    eTeplControl    Tepl[cSTepl];       // параметры зон
    uchar           NFCtr;              // номер контроллера
...
typedef struct eeStrategy
{
    int8_t TempPower;       // приоритет контура
    int8_t RHPower;         // участие в влиянии на влажность в баллах
    int8_t OptimalPower;    // коэф
    int8_t Economic;        // не используется
    int8_t Powers;          // мощьность контура
    int8_t Separate;        // не используется
    int8_t KonturHelp;      // не используется
} eStrategy;
#endif
...
{
    uint16_t    Systems[cConfSSystem];  // кол-во контуров, фрамуг, вентиляторов
    uint16_t    RNum[SUM_NAME_CONF];    // параметры для оборудования в фомате X.YY.ZZ. X - № контроллера, № модуля, ZZ вход/выход
} eMechConfig;
