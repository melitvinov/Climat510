...
unsigned  int checksum(unsigned char *fbuf, unsigned  int len,unsigned char type)
{
    // type 0=ip
    //      1=udp
    //      2=tcp
...
unsigned char check_ip_scr(unsigned char *fbuf,unsigned char *dbuf)
{
    unsigned char i=0;
    unsigned char res=1;
    while (i<4)
    {
        if ((fbuf[IP_SRC_P+i]!=dbuf[i])||(!dbuf[i]))
            res=0;
        dbuf[i]=fbuf[IP_SRC_P+i];
        i++;
    }
    return res;
}
...
void make_tcphead(unsigned char *buf,unsigned  int rel_ack_num,unsigned char mss,unsigned char cp_seq)
{
    ...
    buf[TCP_SRC_PORT_H_P]=myARPport/256;
    buf[TCP_SRC_PORT_L_P]=myARPport%256;
    i=4;
    // sequence numbers:
...
// tcp data. Returns the position at which the string after
// this string could be filled.
unsigned  int fill_tcp_buf(unsigned char *buf,unsigned  int fsize, const char *s)
{
    // fill in tcp data at position pos
    //
    // with no options the data starts after the checksum + 2 more bytes (urgent ptr)
    while (fsize>0)
    {
        buf[TCP_CHECKSUM_L_P+3+fsize-1]=s[fsize-1];
        fsize--;
    }
    return(1);
}
...
// length and checksum
void make_tcp_ack_with_data(unsigned char *buf,unsigned  int dlen, char fin)
{
    unsigned  int j;
    // fill the header:
