...
{
    uint8_t     Cond;
    uint16_t    CpM;    // приведенный адрес датчика
...
uint16_t UpdateInIPC(uint16_t nAddress,TIModulConf* ModulConf)
{
    ...
    vCpM=GetIPCComMod(nAddress);    // vCpM приведенный адрес датчика
    if (!vCpM) return 0;
    if (vCpM/100==6) return 0;      // зарезервированно
    vInput=GetIPCNum(nAddress);
    if (!vInput) return 0;
    for (i=0; i< OUT_MODUL_SUM; i++)
    {
